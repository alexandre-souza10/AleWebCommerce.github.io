...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<header class="entry-header">
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( esc_html__( ', ', 'orionflex' ) );
			if ( $categories_list ) {
	...
	<div class="entry-content">
		...
		if ( is_single() ) {
			the_content(
				sprintf(
					wp_kses(
						...
						__( 'Read More..<span class="screen-reader-text"> "%s"</span>', 'orionflex' ),
						array(
							'span' => array(
								'class' => array(),
							),
			...
			wp_link_pages(
				array(
					'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'orionflex' ),
					'after'  => '</div>',
				)
			);
		} else {
			...
			echo '<br><a class="more-link theme-button" href="' . esc_url( get_permalink( $post->ID ) ) . '">' .
				esc_html__( 'Continue..', 'orionflex' ) . ' <span class="screen-reader-text">' . esc_html( get_the_title( $post->ID ) ) . '</span></a>';
		}
		?>
	</div><!-- .entry-content -->
